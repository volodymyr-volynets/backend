	...
	public function get($type, $options = []) {
		...
		$sql = <<<TTT
			SELECT
				...
				a.sm_menuitm_url,
				-- group 1
				a.sm_menuitm_group1_id g1_id, 
				g1.sm_menugrp_name g1_name,
				g1.sm_menugrp_order g1_order,
				-- group 2
				a.sm_menuitm_group2_id g2_id,
				g2.sm_menugrp_name g2_name,
				g2.sm_menugrp_order g2_order,
				-- group 3
				a.sm_menuitm_group3_id g3_id,
				g3.sm_menugrp_name g3_name,
				g3.sm_menugrp_order g3_order
			FROM [table[numbers_backend_system_menu_model_items]] a
			LEFT JOIN [table[numbers_backend_system_menu_model_groups]] g1 ON a.sm_menuitm_group1_id = g1.sm_menugrp_id
			LEFT JOIN [table[numbers_backend_system_menu_model_groups]] g2 ON a.sm_menuitm_group2_id = g2.sm_menugrp_id
			LEFT JOIN [table[numbers_backend_system_menu_model_groups]] g3 ON a.sm_menuitm_group3_id = g3.sm_menugrp_id
			WHERE 1=1
...
TTT;
		$data = $this->query($sql, null, $this->merge_options($options));
		if (count($data) > 0) {
			$temp = [];
			// loop though data
			foreach ($data as $k => $v) {
