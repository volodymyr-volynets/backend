...
abstract class Base
{
    ...
    protected function process($operation, $migration_id, $name, $data, $options = [])
    {
        ...
        if (isset($this->prior_executed_migration_ids)) {
            ...
            if (empty($this->prior_executed_migration_ids[$migration_id])) {
                return;
            }
        }
        // put value into data
        ...
        if ($this->mode == 'commit') {
            ...
            if ($operation != 'sql_query') {
                ...
                } elseif ($data['type'] == 'schema_delete') {
                    $this->new_objects['schema'][$data['name']] = null;
                } elseif (in_array($data['type'], ['table_new', 'sequence_new'])) {
                    $name = ltrim($data['schema'] . '.' . $data['name'], '.');
                    $type = str_replace('_new', '', $data['type']);
                    $this->new_objects[$type][$name] = $name;
                } elseif (in_array($data['type'], ['extension_new', 'function_new', 'view_new'])) { // backend specific
                    $name = ltrim($data['schema'] . '.' . $data['name'], '.');
                    $type = str_replace('_new', '', $data['type']);
                ...
                } elseif (in_array($data['type'], ['table_delete', 'sequence_delete', 'extension_delete', 'function_delete', 'view_delete'])) {
                    $name = ltrim($data['schema'] . '.' . $data['name'], '.');
    ...
    public function execute($type = 'up', $options = [])
    {
        ...
        try {
            ...
            if ($migration_model->dbPresent()) {
                ...
                $temp_result = Migrations::collectionStatic(['skip_acl' => true])->merge($this->executed_migration_stats);
