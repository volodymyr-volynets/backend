...
class DDL
{
    ...
    public function objectRemove($object, $db_link)
    {
        ...
        } elseif (in_array($object['type'], ['function', 'extension', 'trigger', 'view', 'check'])) {
        ...
        } elseif (in_array($object['type'], ['column_delete'])) {
    ...
    public function processTableModel($model_class, $options = [])
    {
        ...
        do {
            ...
            // columns
            $processed_columns = Common::processDomainsAndTypes($model->columns);
            $attributes = array_merge(Common::$attributes['global'], Common::$attributes['schema']);
            $columns = [];
            ...
            if ($model->periods['type'] != 'none') {
                ...
                $generated_classes = File::iterate($pathinfo['dirname'], [
            ...
            if (!empty($model->constraints)) {
                foreach ($model->constraints as $k => $v) {
                    ...
                    if ($v['type'] == 'fk') {
                        ...
                        if (empty($v['options']['match'])) {
                            $v['options']['match'] = 'simple';
                        }
                        if (empty($v['options']['update'])) {
                            $v['options']['update'] = 'cascade';
                        }
                        if (empty($v['options']['delete'])) {
                            $v['options']['delete'] = 'restrict';
                        }
                    }
                    // add constraint
                    $this->objectAdd([
    ...
    public function compareSchemas($obj_master, $obj_slave, $options = [])
    {
        ...
        if (!empty($obj_master['extension'])) {
            foreach ($obj_master['extension'] as $k => $v) {
                ...
                if ($options['type'] == 'schema' && $k != $options['backend']) {
                    continue;
                }
                foreach ($v as $k2 => $v2) {
                    foreach ($v2 as $k3 => $v3) {
        ...
        if (!empty($obj_master['table'])) {
            foreach ($obj_master['table'] as $k => $v) {
                foreach ($v as $k2 => $v2) {
                    ...
                    if (!empty($result['up']['new_tables'][$v2['data']['full_table_name']])) {
                        continue;
                    }
                    // finding new column
                    foreach ($v2['data']['columns'] as $k3 => $v3) {
                        ...
                        if (empty($obj_slave['table'][$k][$k2]['data']['columns'][$k3])) {
                            ...
                            foreach ($compare_columns as $v88) {
                                ...
                                } elseif ($master_compare[$v88] !== $slave_compare[$v88]) {
                                    $temp_error = true;
                                    break;
                                }
                            }
                        ...
                        if (empty($obj_slave['constraint'][$k][$k2][$k3])) {
                            ...
                            if ($v3['data']['type'] == 'fk') {
                                ...
                                if ($options['backend'] != 'Oracle') {
                                    if ($v3['data']['options']['match'] !== $obj_slave['constraint'][$k][$k2][$k3]['data']['options']['match']) {
                                        $temp_error = true;
                                    }
                                    if ($v3['data']['options']['update'] !== $obj_slave['constraint'][$k][$k2][$k3]['data']['options']['update']) {
                                        $temp_error = true;
                                    }
                                    if ($v3['data']['options']['delete'] !== $obj_slave['constraint'][$k][$k2][$k3]['data']['options']['delete']) {
                                        $temp_error = true;
                                    }
                                }
                            } else { // pk and unique
