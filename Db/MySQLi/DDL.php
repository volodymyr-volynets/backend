...
use Numbers\Backend\Db\Common\Model\Metadata;
                            ...
                            foreach ($v2 as $k3 => $v3) {
                                foreach ($v3 as $k4 => $v4) {
                                    ...
                                    if ($default !== null) {
                                        ...
                                        } elseif (is_string($default) && strpos($default, 'CURRENT_TIMESTAMP') !== false) {
                                            $default = 'now()';
                                        }
                                    }
                            ...
                            foreach ($v2 as $k3 => $v3) {
                                foreach ($v3 as $k4 => $v4) {
                                    foreach ($v4 as $k5 => $v5) {
                                        ...
                                        } elseif ($v5['constraint_type'] == 'UNIQUE') {
                                            $temp2 = [
                                                'type' => 'unique',
                                                'columns' => explode(',', $v5['column_names']),
                                                'full_table_name' => $full_table_name
                                            ];
                                        } elseif ($v5['constraint_type'] == 'INDEX') {
                                            $temp2 = [
                                        ...
                                        } elseif ($v5['constraint_type'] == 'FOREIGN KEY') {
    ...
    public function renderSql($type, $data, $options = [], & $extra_comments = null)
    {
        ...
        switch ($type) {
            ...
            case 'view_new':
                ...
                $result = array_merge($result, Metadata::makeSchemaChanges($options['db_link'], 'view', $data['data']['full_view_name'], $data['data']['sql_version'], false));
                break;
            case 'view_delete':
                $result = [];
                $result[] = "DROP VIEW {$data['data']['full_view_name']};";
                $result = array_merge($result, Metadata::makeSchemaChanges($options['db_link'], 'view', $data['data']['full_view_name'], '', true));
                break;
...
TTT;
            ...
            case 'function_new':
                ...
                $result = array_merge($result, Metadata::makeSchemaChanges($options['db_link'], 'function', $data['data']['full_function_name'], $data['data']['sql_version'], false));
                break;
            case 'function_delete':
                $result = [];
                $result[] = "DROP FUNCTION {$data['data']['full_function_name']};";
                $result = array_merge($result, Metadata::makeSchemaChanges($options['db_link'], 'function', $data['data']['full_function_name'], '', true));
                break;
                // trigger
            case 'trigger_new':
                $result = [];
                $result[] = trim($data['data']['definition']);
                $result = array_merge($result, Metadata::makeSchemaChanges($options['db_link'], 'trigger', $data['data']['full_function_name'], $data['data']['sql_version'], false));
                break;
            case 'trigger_delete':
                $result = [];
                $result[] = "DROP TRIGGER {$data['data']['full_function_name']};";
                $result = array_merge($result, Metadata::makeSchemaChanges($options['db_link'], 'trigger', $data['data']['full_function_name'], '', true));
                break;
                // check
            case 'check_new':
                $result = [];
                $result = array_merge($result, $data['data']['definition']);
                $result = array_merge($result, Metadata::makeSchemaChanges($options['db_link'], 'check', $data['data']['full_check_name'], $data['data']['sql_version'], false));
                break;
            case 'check_delete':
                $result = [];
                $result[] = "DROP TRIGGER {$data['data']['full_check_name']}_insert;";
                $result[] = "DROP TRIGGER {$data['data']['full_check_name']}_update;";
                $result = array_merge($result, Metadata::makeSchemaChanges($options['db_link'], 'check', $data['data']['full_check_name'], '', true));
                break;
