...
use Helper\Cmd;
use Numbers\Backend\Db\Oracle\Model\Sequence\Currval;
use Numbers\Backend\Db\Oracle\Model\Sequence\Nextval;
                ...
                while ($rows = oci_fetch_assoc($resource)) {
                    foreach ($rows as $k => $v) {
                        ...
                        if (is_string($v) && $v == '__EMPTY_STR__') {
                            $rows[$k2] = '';
                        } elseif ($result['structure'][$k2]['type'] == 'number') { // numbers
                            if (!is_null($v)) {
                                if ((intval($v) . '') == $v) {
                                    $rows[$k2] = (int) $v;
                                } else {
                        ...
                        } elseif (in_array($result['structure'][$k2]['type'], ['clob', 'blob', 'nclob'])) { // texts
                            if (is_object($v)) {
...
TTT;
    ...
    private function prepareOracleFixes(& $options)
    {
        foreach ($options as $k => $v) {
            ...
            } elseif ((is_string($v) || is_array($v)) && strlen($encoded_v) > 1000) {
                $result = [];
                $key[1] = $key[1] ?? '';
                $key[2] = '~~';
