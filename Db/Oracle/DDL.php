...
class DDL extends \Numbers\Backend\Db\Common\DDL implements \Numbers\Backend\Db\Common\Interface2\DDL
{
    ...
    public function loadSchema($db_link)
    {
        ...
        foreach ($ddl_objects as $v) {
                ...
                switch ($v) {
                            ...
                            foreach ($v2 as $k3 => $v3) {
                                foreach ($v3 as $k4 => $v4) {
                                    ...
                                    if (isset($default)) {
                                        ...
                                        if ($default == 'NULL') {
                                            $default = null;
                                        } elseif (is_numeric($default) && $v4['type'] == 'number') {
                                            $default = $default * 1;
                                        }
                                    }
                                    // serial types
                                    if ($v4['type'] == 'number') {
                                        if (strpos($v4['comments'], 'sequence') !== false) {
                                            if ($v4['precision'] == 5) {
                                                $type = 'smallserial';
                                            } elseif ($v4['precision'] == 10) {
                                                $type = 'serial';
                                            } elseif ($v4['precision'] == 19) {
                                                $type = 'bigserial';
                                            }
                                            $sequence = true;
                                            // we need to zero out
                                            ...
                                            $v4['precision'] = 0;
                                            $v4['scale'] = 0;
                                        } elseif (strpos($v4['comments'], 'bcnumeric') !== false) {
                                            $type = 'bcnumeric';
                                            if (!empty($default)) {
                                                $default = trim($default, "'");
                                            }
                                        } elseif (strpos($v4['comments'], 'numeric') !== false) {
                                            $type = 'numeric';
                                            $sql_type = 'number';
                                            if (!empty($default)) {
                                                $default = trim($default, "'");
                                        ...
                                        $v4['length'] = 0;
                                    } elseif ($v4['type'] == 'varchar2') { // varchar
                                        $type = 'varchar';
                                    } elseif (in_array($v4['type'], ['clob', 'blob'])) { // text, json
                    ...
                    case 'sequences':
                        ...
                        if ($sequence_model->dbPresent()) {
                            $sequence_attributes = $sequence_model->get(['skip_acl']);
                        }
                        // add sequences
                        foreach ($temp['data'] as $k2 => $v2) {
                            foreach ($v2 as $k3 => $v3) {
            ...
            $counter++;
            // update progress bar
            Cmd::progressBar(25 + round($counter / count($ddl_objects) * 100 / 4, 0), 100, 'Loading DB objects');
        }
...
TTT;
            ...
            case 'column_new':
                ...
                if (is_string($default) && $default != 'now()') {
                    $default = "'" . $default . "'";
                } elseif (is_string($default) && $default == 'now()') {
