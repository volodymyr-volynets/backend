...
class Base
{
     ...
     */
    public static function render(Report & $object, string $report_name): string
    {
        Base::add();
        $result = '';
        ...
        foreach ($object->data[$report_name]['data'] as $row_number => $row_data) {
            ...
            foreach ($header as $k2 => $v2) {
                ...
                $config['data']['datasets'][$current_index]['backgroundColor'][] = Colors::colorFromString($config['data']['datasets'][$current_index]['label'] . ' - ' . $row_data[0][$v2['__index']]);
            }
        ...
        if (!empty($other_values)) {
            ...
            foreach ($other_values as $k => $v) {
                ...
                $config['data']['datasets'][$current_index]['backgroundColor'][] = Colors::colorFromString($last_label);
            }
        }
